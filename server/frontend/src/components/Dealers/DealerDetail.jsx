...
export default function DealerDetail() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const dRes = await fetch(`/djangoapp/dealer/${dealer_id}/`);
        const dJson = await dRes.json();
        if (dJson.status === 200) {
          const d = Array.isArray(dJson.dealer) ? dJson.dealer[0] : dJson.dealer;
          setDealer(d);
        }
        const rRes = await fetch(`/djangoapp/reviews/dealer/${dealer_id}/`);
  ...
  return (
    <div>
      ...
      <div style={{ margin: "2rem" }}>
        ...
        {isLoggedIn && (
          <p>
            <Link to={`/postreview/${dealer.id}/`} className="postreview">Post Review</Link>
          </p>
        )}
